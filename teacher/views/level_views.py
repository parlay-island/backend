...
from teacher.views.view_config import LEVEL, LEVEL_ID, USER, GET, POST, DELETE, DISTANCE, PAGE, PAGE_SIZE, RESULTS, LEVELS, \
    NAME
...
def get_results_by_level(request, level, user):
    results = Result.objects.filter(level=Level.objects.get(id=level),
        ...
        .order_by('-%s' % DISTANCE) if len(Level.objects.filter(id=level)) > 0 else []
